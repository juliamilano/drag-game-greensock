...
TweenMax.set([".instruction"], {perspective: 400, textShadow: "2px 2px 10px rgba(0,0,0, .6)"});
...
function initTitle(){
    for(let i = 0; i < numWords; i++){
        tl.from(
            titleText.chars[i], 0.5, {
                force3d: true,
                opacity:0,
                x: -500,
                'color': 'orange',
                'text-shadow': '0px 0px 5px rgba(255, 255, 185, 0.8)',
            ...
            if(spotID===dragID){
                if(dragThing.hitTest(spot, '10%')){
                    ...
                    TweenMax.to(".correct", .5, {
                        ...
                        autoAlpha: 1,
                        onComplete: function(){
                            TweenMax.to(".correct", 0.2, {scale: 0, autoAlpha: 0, delay: 0.5})
                        }
                    });
                   ...
                   TweenMax.to(".tryAgain", .5, {
                        scale: 2,
                        autoAlpha: 1,
                        onComplete: function(){
                            TweenMax.to(".tryAgain", 0.2, {scale: 0, autoAlpha: 0, delay: 0.5})
                        }
                    });
                    TweenMax.to(dragThing.target, .5, {
                        x: dragThing.startX,
